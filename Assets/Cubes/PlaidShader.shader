Shader "Custom/PlaidShader" {
	...
	SubShader {
		...
		float4 makePlaid (float2 coordinates, float2 pattern_offset, float lines, float stripe_size, float stripe_fuzziness) {
			float4 result;
			result = cutoff(sin((pattern_offset.x + coordinates.x) * lines * 4), stripe_size, stripe_fuzziness) / 2;
			result += cutoff(sin((pattern_offset.y + coordinates.y) * lines * 4), stripe_size, stripe_fuzziness) / 2;
			return result; 
		}
		
		void surf (Input IN, inout SurfaceOutput o) {
			float4 pattern = makePlaid(IN.full_uv, _Time.x * _StripeSpeed, _Stripes, _StripeSizeMin + (_StripeSize - _StripeSizeMin) * (cos(_Time.xx * _StripeSizeSpeed) + 1) * 0.5, _StripeFuzziness);
